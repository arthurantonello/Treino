...
-- e abaixo do patamar médio do período.
  ...
  SELECT PCPRODUT.CODFORNEC,
  		 ROUND(SUM(PCPEDI.PVENDA * PCPEDI.QT), 2) 		AS SOMA,
  		 'Acima da média' 								AS STATUS
    FROM WINT.PCPEDI
    JOIN WINT.PCPRODUT
      ON PCPEDI.CODPROD = PCPRODUT.CODPROD
   WHERE PCPEDI.POSICAO = 'F'
...
GROUP BY PCPRODUT.CODFORNEC
 HAVING SUM(PCPEDI.PVENDA * PCPEDI.QT) > (
						  SELECT AVG(SOMA_TOTAL.TOTAL_SOMA) MEDIA_PERIODO
						    FROM (
								   SELECT SUM(SUBPCPEDI.PVENDA * SUBPCPEDI.QT) AS TOTAL_SOMA
								     FROM WINT.PCPEDI SUBPCPEDI
								     JOIN WINT.PCPRODUT SUBPCPRODUT
								       ON SUBPCPEDI.CODPROD = SUBPCPRODUT.CODPROD
							        WHERE SUBPCPEDI.POSICAO = 'F'
...
/*CONSULTA QUE TRAGA A MÉDIA DE VENDA DOS PRODUTOS DA MARCA VERSOTOOLS, O VALOR TOTAL
VENDIDO DE CADA PRODUTO, A ÚLTIMA DATA DE COMPRA, QUANTIDADE MÉDIA POR PEDIDO,
QUANTIDADE DE CLIENTES E TOTAL DE PEDIDOS.
REGRAS:
PEDIDO FATURADO, ÚLTIMOS 90 DIAS;
RESULTADO ESPERADO:
CODIGO_PRODUTO | DESCRICAO | MARCA | DT_ULT_PEDIDO | MEDIA_VENDAS | VL_TOTAL_VENDIDO |
QTD_MEDIA_POR_PEDIDO | QTD_CLIENTES | TOTAL_PEDIDOS */

  SELECT PCPRODUT.CODPROD  									AS CODIGO_PRODUTO,
  		 PCPRODUT.DESCRICAO,
  		 PCPRODUT.MARCA,
  		 MAX(PCPEDI.DATA)									AS DT_ULT_PEDIDO,
  		 ROUND(AVG(PCPEDI.PVENDA * PCPEDI.QT), 2)  			AS MEDIA_VENDAS,
  		 ROUND(SUM(PCPEDI.PVENDA * PCPEDI.QT), 2)			AS VL_TOTAL_VENDIDO,
  		 ROUND(AVG(PCPEDI.QT), 2) 							AS QTD_MEDIA_POR_PEDIDO,
  		 COUNT(DISTINCT PCPEDI.CODCLI)						AS QTD_CLIENTES,
  		 COUNT(DISTINCT PCPEDI.NUMPED)						AS TOTAL_PEDIDOS
    FROM WINT.PCPEDI
    JOIN WINT.PCPRODUT
      ON PCPEDI.CODPROD = PCPRODUT.CODPROD
     AND PCPRODUT.MARCA = 'VERSOTOOLS'
   WHERE PCPEDI.POSICAO = 'F'
     AND PCPEDI.DATA BETWEEN SYSDATE - 90
     					 AND SYSDATE
GROUP BY PCPRODUT.CODPROD,
  		 PCPRODUT.DESCRICAO,
  		 PCPRODUT.MARCA

/* CONSULTA QUE TRAGA A MAIOR VENDA DE CADA REPRESENTANTE DO GRUPO, MÉDIA DE VENDAS,
QUANTIDADE DE CLIENTES POSITIVADOS, QUANTIDADE DE PEDIDOS, MÉDIA DE PRODUTOS / PEDIDO,
QUANTIDADE TOTAL DE PRODUTOS, QUANTIDADE DE DIAS POSITIVADOS E ÚLTIMO PEDIDO, ORDENANDO
POR ORDEM ALFABÉTICA.
REGRAS:
PEDIDO FATURADO, VALOR TOTAL DE VENDAS > R$ 2000,00;
RESULTADO ESPERADO:
CODUSUR | NOME | CODSUPERVISOR | NOME_SUP | VALOR_MEDIO | QTD_CLIENTES | MAIOR_VENDA |
QTD_TOTAL_PEDIDOS | SKUS_PEDIDO | QTD_PRODUTOS | QTD_DIAS | ULTIMO_PEDIDO */

  SELECT PCUSUARI.CODUSUR,
  		 PCUSUARI.NOME,
  		 PCSUPERV.CODSUPERVISOR,
  		 PCSUPERV.NOME						  AS NOME_SUP,
  		 ROUND(AVG(PCPEDC.VLTOTAL), 2)		  AS VALOR_MEDIO,
  		 COUNT(DISTINCT PCPEDC.CODCLI)		  AS QTD_CLIENTES,
  		 MAX(PCPEDC.VLTOTAL)				  AS MAIOR_VENDA,
  		 COUNT(DISTINCT PCPEDC.NUMPED)		  AS QTD_TOTAL_PEDIDOS,
  		 COUNT(DISTINCT PCPEDI.CODPROD)		  AS SKUS_PEDIDO,
  		 ROUND(SUM(PCPEDI.QT), 2)			  AS QTD_PRODUTOS,
  		 COUNT(DISTINCT PCPEDC.DATA) 		  AS QTD_DIAS,
  		 MAX(PCPEDC.NUMPED)					  AS ULTIMO_PEDIDO
    FROM WINT.PCUSUARI
  	JOIN WINT.PCSUPERV
      ON PCUSUARI.CODSUPERVISOR = PCSUPERV.CODSUPERVISOR
    JOIN WINT.PCPEDC
...
GROUP BY PCUSUARI.CODUSUR,
  ...
  SELECT PCUSUARI.*
    FROM WINT.PCUSUARI
  ORDER BY PCUSUARI.CODUSUR ASC;

  -- Liste os produtos que tiveram a menor venda total em out/nov de 2011.
  ...
  SELECT PCPRODUT.CODPROD,
   ...
   WHERE PCPRODUT.CODPROD IN( 
   								  SELECT VENDAS_POR_PRODUTO.CODPROD
   								    FROM (
											  SELECT PCPEDI.CODPROD					AS CODPROD,
											  		 SUM(PCPEDI.PVENDA * PCPEDI.QT) AS TOTAL_VENDA_PRODUTO
											    FROM WINT.PCPEDI
											   WHERE PCPEDI.POSICAO = 'F'
											     AND PCPEDI.DATA BETWEEN TO_DATE('01/10/2011', 'DD/MM/YYYY')
											   						 AND TO_DATE('30/11/2011', 'DD/MM/YYYY')
											GROUP BY PCPEDI.CODPROD
											ORDER BY TOTAL_VENDA_PRODUTO ASC
											FETCH FIRST 10 ROWS ONLY) VENDAS_POR_PRODUTO
   								    );
   
-- Identifique os clientes que realizaram o maior pedido individual (de maior valor) em 2024.
  

  SELECT PCPEDC.CODCLI,
    	 PCPEDC.VLTOTAL				AS VLTOTAL
    FROM WINT.PCPEDC
   WHERE PCPEDC.POSICAO = 'F'
     AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2024
     AND PCPEDC.VLTOTAL = (
     						 SELECT MAX(PCPEDC.VLTOTAL)
     						   FROM WINT.PCPEDC
     						  WHERE PCPEDC.POSICAO = 'F'
     							AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2024
     					  )
ORDER BY PCPEDC.CODCLI ASC; -- EM CASO DE EMPATE
  

-- Retorne os representantes que não possuem nenhum pedido registrado no ano de 2023.

  SELECT *
    FROM WINT.PCUSUARI USUARIO
   WHERE NOT EXISTS(SELECT * 
  					  FROM WINT.PCPEDC
  					 WHERE PCPEDC.CODUSUR = USUARIO.CODUSUR
  					   AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2023);


-- Exiba, para cada mês de 2023, o total vendido e o maior pedido registrado naquele mês.
  
  
  SELECT EXTRACT(MONTH FROM PCPEDC.DATA) 	AS MES,
  		 SUM(PCPEDC.VLTOTAL) 				AS TOTAL_VENDIDO,
  		 (
  		 	SELECT MAX(SUBPCPEDC.VLTOTAL)
  		 	  FROM WINT.PCPEDC SUBPCPEDC
  		 	 WHERE SUBPCPEDC.POSICAO = 'F'
  		 	   AND EXTRACT(YEAR FROM SUBPCPEDC.DATA) = 2023
  		 	   AND EXTRACT(MONTH FROM PCPEDC.DATA) = EXTRACT(MONTH FROM SUBPCPEDC.DATA)
  		 )									AS MAIOR_PEDIDO
    FROM WINT.PCPEDC 
   WHERE PCPEDC.POSICAO = 'F'
     AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2023
...
-- Liste os produtos que nunca foram vendidos em out/nov de 2011. Exiba o código e a descrição dos produtos.

-- Se fosse apenas código:
  SELECT PCPRODUT.CODPROD
    FROM WINT.PCPRODUT
   MINUS
  SELECT PCPEDI.CODPROD
...
-- Com subselect
  SELECT PCPRODUT.CODPROD,
  		 PCPRODUT.DESCRICAO
    FROM WINT.PCPRODUT
   WHERE PCPRODUT.CODPROD NOT IN (
			   						SELECT PCPEDI.CODPROD
			   						  FROM WINT.PCPEDI
			   					)
   

-- Retorne os representantes que tiveram a menor venda total (considerando VLTOTAL) no ano de 2023.
-- Exiba o código do representante e o valor total vendido.
		
	-- Se for exatamente a menor venda
  SELECT PCPEDC.CODUSUR,
  		 SUM(PCPEDC.VLTOTAL) AS TOTAL_VENDIDO
    FROM WINT.PCPEDC
   WHERE PCPEDC.POSICAO = 'F'
...
GROUP BY PCPEDC.CODUSUR
  HAVING SUM(PCPEDC.VLTOTAL) = (
  							SELECT MIN(VENDA_REPRESENTANTE.TOTAL)
  							  FROM (
						  			  SELECT SUM(SUBPCPEDC.VLTOTAL) 		AS TOTAL
						  			    FROM WINT.PCPEDC SUBPCPEDC
						  			   WHERE SUBPCPEDC.POSICAO = 'F'
						  			     AND EXTRACT(YEAR FROM SUBPCPEDC.DATA) = 2023
						  			 GROUP BY SUBPCPEDC.CODUSUR
...
-- Liste os clientes que, em 2024, fizeram pedidos com valor superior ao maior pedido feito por qualquer cliente em 2023.
-- Exiba o código do cliente e o valor do pedido.
...
-- Traga os fornecedores que, em out/nov de 2011, venderam exclusivamente produtos de uma única marca.
-- Exiba o código do fornecedor e a marca.
  
  SELECT PCFORNEC.CODFORNEC,
  		 MIN(PCPRODUT.MARCA) AS MARCA
    FROM WINT.PCFORNEC
    JOIN WINT.PCPRODUT
      ON PCFORNEC.CODFORNEC = PCPRODUT.CODFORNEC
    JOIN WINT.PCPEDI
     ...
     AND PCPEDI.DATA BETWEEN TO_DATE('01/10/2011', 'DD/MM/YYYY')
   						 AND TO_DATE('30/11/2011', 'DD/MM/YYYY')
  GROUP BY PCFORNEC.CODFORNEC
  HAVING COUNT(DISTINCT PCPRODUT.MARCA) = 1;
  

   

-- Exiba os produtos que foram vendidos em out/nov de 2011, mas que não possuem mais cadastro ativo no PCPRODUT.
  ...
  SELECT PCPEDI.CODPROD
   ...
   WHERE PCPEDI.POSICAO = 'F'
     AND PCPEDI.DATA BETWEEN TO_DATE('01/10/2011', 'DD/MM/YYYY') 
                   		 AND TO_DATE('30/11/2011', 'DD/MM/YYYY')
     AND PCPEDI.CODPROD NOT IN (
   								 SELECT PCPRODUT.CODPROD
   								   FROM WINT.PCPRODUT
   							   );


-- Liste os clientes que, em 2024, fizeram pedidos com valor total inferior ao valor do menor pedido de 2024.
-- Exiba o código do cliente e o valor total comprado no ano.
  
  SELECT PCPEDC.CODCLI,
  		 SUM(PCPEDC.VLTOTAL) TOTAL_COMPRADO
    FROM WINT.PCPEDC
   WHERE PCPEDC.POSICAO = 'F'
     AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2024
GROUP BY PCPEDC.CODCLI
  HAVING SUM(PCPEDC.VLTOTAL) < (
     						SELECT MIN(SUBPCPEDC.VLTOTAL)
     						  FROM WINT.PCPEDC SUBPCPEDC
     						 WHERE SUBPCPEDC.POSICAO = 'F'
     						   AND EXTRACT(YEAR FROM SUBPCPEDC.DATA) = 2024);


-- Apresente os representantes que venderam no ano de 2024, mas que não venderam em nenhum mês de 2023.
-- Exiba o código do representante.
  
  SELECT DISTINCT PCPEDC.CODUSUR
    FROM WINT.PCPEDC
   WHERE PCPEDC.POSICAO = 'F'
     AND EXTRACT(YEAR FROM PCPEDC.DATA) = 2024
     AND PCPEDC.CODUSUR NOT IN (
								  SELECT DISTINCT SUBPCPEDC.CODUSUR
								    FROM WINT.PCPEDC SUBPCPEDC
								   WHERE SUBPCPEDC.POSICAO = 'F'
								     AND EXTRACT(YEAR FROM SUBPCPEDC.DATA) = 2023);
  

-- Liste os clientes que fizeram pedidos em todos os meses de 2024.
-- Exiba o código do cliente e a quantidade de meses com vendas.
  
  SELECT PCPEDC.CODCLI,
  		 COUNT(EXTRACT (MONTH FROM PCPEDC.DATA)) AS MESES_POSITIVADOS
    FROM WINT.PCPEDC
   WHERE EXTRACT(YEAR FROM PCPEDC.DATA) = 2024
GROUP BY PCPEDC.CODCLI
  HAVING COUNT(EXTRACT (MONTH FROM PCPEDC.DATA)) = 12;


-- Mostre os fornecedores que venderam no mínimo dois produtos diferentes em out/nov de 2011.
-- Exiba o código do fornecedor e a quantidade de produtos distintos vendidos.
  
  SELECT PCPRODUT.CODFORNEC,
  		 COUNT(PCPRODUT.CODPROD)
    FROM WINT.PCPEDI
    JOIN WINT.PCPRODUT
      ON PCPEDI.CODPROD = PCPRODUT.CODPROD
   WHERE PCPEDI.POSICAO = 'F'
     AND PCPEDI.DATA BETWEEN TO_DATE('01/10/2011', 'DD/MM/YYYY') 
                   		 AND TO_DATE('30/11/2011', 'DD/MM/YYYY')
GROUP BY PCPRODUT.CODFORNEC
  HAVING COUNT(DISTINCT PCPRODUT.CODPROD) >= 2;

/*CONSULTA QUE TRAGA SOMENTE OS PRODUTOS QUE POSSUEM FORNECEDOR
CADASTRADO. APRESENTANDO, PRODUTO EM ORDEM ALFABÉTICA, MARCA E
FORNECEDOR;*/
...
/*CONSULTA QUE TRAGA SOMENTE OS REPRESENTANTES QUE POSSUEM
SUPERVISOR VINCULADO NA TABELA DE SUPERVISORES, POR ORDEM ALFABÉTICA;*/

  SELECT PCUSUARI.NOME	AS NOME_REPR,
  	     PCSUPERV.NOME 	AS NOME_SUP
    FROM WINT.PCUSUARI
    JOIN WINT.PCSUPERV
      ON PCUSUARI.CODSUPERVISOR = PCSUPERV.CODSUPERVISOR
ORDER BY NOME_REPR ASC,
		 NOME_SUP ASC;

/*CONSULTA QUE TRAGA TODOS OS PEDIDOS E A QUANTIDADE DE ITENS DE CADA
PEDIDO, ORDENANDO POR PEDIDOS COM MAIOR QUANTIDADE PRIMEIRO.
(APENAS NUMERO DO PEDIDO E QUANTIDADE DE ITENS)*/
...
-- CONSULTA QUE TRAGA A SOMA DE VENDA, QUANTIDADE DE DIAS POSITIVADOS, QUANTIDADE DE CLIENTES E QUANTIDADE TOTAL VENDIDA DOS PRODUTOS DA MARCA VERSOTOOLS QUE TENHAM UM VALOR DE VENDA MAIOR QUE R$ 2.000,00;

 /*
  * Soma vendas
  * qtd dias positivados
  * qtd clientes
  * qtd total vendida
  * 
  * Regras:
  * Produto VERSOTOOLS
  * Produtos com valor de venda > 2000
  */

  SELECT PCPEDI.CODPROD,
  		 SUM(PCPEDI.PVENDA * PCPEDI.QT)		AS SOMA_VENDAS,
  		 COUNT(DISTINCT PCPEDI.DATA)		AS DIAS_POSITIVADOS,
  		 COUNT(DISTINCT PCPEDI.CODCLI)		AS QTD_CLIENTES,
  		 SUM(PCPEDI.QT)						AS QTD_TOTAL_VENDIDA
    FROM WINT.PCPEDI
    JOIN WINT.PCPRODUT
      ON PCPEDI.CODPROD = PCPRODUT.CODPROD
         AND PCPRODUT.MARCA = 'VERSOTOOLS'
   WHERE PCPEDI.PVENDA > 2000
GROUP BY PCPEDI.CODPROD;


   
-- CONSULTA QUE TRAGA A VENDA TOTAL, TOTAL DE CLIENTES POSITIVADOS, MEDIA DE VALOR POR REPRESENTANTE E MEDIA DE DIAS POR REPRESENTANTE DE TODOS OS SUPERVISORES COM VENDA MENOR QUE R$ 10.000,00

  SELECT PCSUPERV.CODSUPERVISOR,
  		 PCSUPERV.NOME															AS NOME_SUP,
  		 SUM(PCPEDC.VLTOTAL)													AS VENDA_TOTAL,
  		 COUNT(DISTINCT PCPEDC.CODCLI)											AS QTD_CLIENTES,
  		 ROUND(AVG(PCPEDC.VLTOTAL), 2)											AS MEDIA_VALOR,
  		 COUNT(DISTINCT PCPEDC.DATA) / COUNT(DISTINCT PCPEDC.CODUSUR)			AS MEDIA_DIAS
    FROM WINT.PCUSUARI
    JOIN WINT.PCSUPERV
      ON PCUSUARI.CODSUPERVISOR = PCSUPERV.CODSUPERVISOR
    JOIN WINT.PCPEDC
      ON PCUSUARI.CODUSUR = PCPEDC.CODUSUR
GROUP BY PCSUPERV.CODSUPERVISOR,
  		 PCSUPERV.NOME
  HAVING SUM(PCPEDC.VLTOTAL) < 10000
   UNION
  SELECT PCPEDC.CODSUPERVISOR,
  		 'Supervisor sem nome'													AS NOME_SUP,
  		 SUM(PCPEDC.VLTOTAL)													AS VENDA_TOTAL,
  		 COUNT(DISTINCT PCPEDC.CODCLI)											AS QTD_CLIENTES,
  		 ROUND(AVG(PCPEDC.VLTOTAL), 2)											AS MEDIA_VALOR,
  		 COUNT(DISTINCT PCPEDC.DATA) / COUNT(DISTINCT PCPEDC.CODUSUR)			AS MEDIA_DIAS
...
GROUP BY PCPEDC.CODSUPERVISOR
  ...
  /* 
 * Enunciado:
 * CONSULTA QUE TRAGA A MÉDIA DE VENDA DOS PRODUTOS DA MARCA VERSOTOOLS, O VALOR TOTAL VENDIDO DE CADA PRODUTO, A ÚLTIMA DATA DE COMPRA, QUANTIDADE MÉDIA POR PEDIDO, QUANTIDADE DE CLIENTES E TOTAL DE PEDIDOS. 
 * REGRAS: PEDIDO FATURADO, ÚLTIMOS 90 DIAS;
 * RESULTADO ESPERADO: CODIGO_PRODUTO | DESCRICAO | MARCA | DT_ULT_PEDIDO | MEDIA_VENDAS | VL_TOTAL_VENDIDO | QTD_MEDIA_POR_PEDIDO | QTD_CLIENTES | TOTAL_PEDIDOS
*/
  
  SELECT PCPRODUT.CODPROD																AS CODIGO_PRODUTO,
  		 PCPRODUT.DESCRICAO																AS DESCRICAO,
  		 PCPRODUT.MARCA																	AS MARCA,
  		 MAX(PCPEDI.DATA)																AS DT_ULT_PEDIDO,
  		 ROUND(AVG(PCPEDI.PVENDA * PCPEDI.QT), 2)										AS MEDIA_VENDAS,
  		 ROUND(SUM(PCPEDI.PVENDA * PCPEDI.QT), 2)										AS VL_TOTAL_VENDIDO,
  		 ROUND(SUM(PCPEDI.QT) / COUNT(DISTINCT PCPEDI.NUMPED), 2)						AS QTD_MEDIA_POR_PEDIDO,
  		 COUNT(DISTINCT PCPEDI.CODCLI)													AS QTD_CLIENTES,
  		 COUNT(DISTINCT PCPEDI.NUMPED)													AS TOTAL_PEDIDOS
    FROM WINT.PCPEDI
    JOIN WINT.PCPRODUT
...
/* 
 * Enunciado:
 * CONSULTA QUE TRAGA A MAIOR VENDA DE CADA REPRESENTANTE DO GRUPO, MÉDIA DE VENDAS, QUANTIDADE DE CLIENTES POSITIVADOS, QUANTIDADE DE PEDIDOS, MÉDIA DE PRODUTOS / PEDIDO, QUANTIDADE TOTAL DE PRODUTOS, QUANTIDADE DE DIAS POSITIVADOS E ÚLTIMO PEDIDO, ORDENANDO POR ORDEM ALFABÉTICA.
 * REGRAS:PEDIDO FATURADO, VALOR TOTAL DE VENDAS > R$ 2000,00;
 * RESULTADO ESPERADO:CODUSUR | NOME | CODSUPERVISOR | NOME_SUP | VALOR_MEDIO | QTD_CLIENTES | MAIOR_VENDA | QTD_TOTAL_PEDIDOS | SKUS_PEDIDO | QTD_PRODUTOS | QTD_DIAS | ULTIMO_PEDIDO
 */
  ...
  SELECT PCUSUARI.CODUSUR,
  		 ...
  		 PCSUPERV.CODSUPERVISOR,
  		 PCSUPERV.NOME 										 	AS NOME_SUP,
  		 ROUND(AVG(PCPEDI.PVENDA * PCPEDI.QT), 2) 				AS VALOR_MEDIO,
  		 ROUND(AVG(PCPEDC.VLTOTAL), 2) 							AS VALOR_MEDIO2,    -- PELA PCPEDC (PROBLEMA NA TABELA)
  		 COUNT(DISTINCT PCPEDC.CODCLI) 							AS QTD_CLIENTES,
  		 COUNT(DISTINCT PCPEDI.CODCLI) 							AS QTD_CLIENTES2,   -- PELA PCPEDC (PROBLEMA NA TABELA)
  		 MAX(PCPEDI.PVENDA * PCPEDI.QT)							AS MAIOR_VENDA,
  		 MAX(PCPEDC.VLTOTAL)									AS MAIOR_VENDA2, 	-- PELA PCPEDC (PROBLEMA NA TABELA)
  		 COUNT(DISTINCT PCPEDI.NUMPED)							AS QTD_TOTAL_PEDIDOS,
  		 COUNT(DISTINCT PCPEDI.CODPROD)							AS SKUS_PEDIDO,
  		 SUM(PCPEDI.QT)											AS QTD_PRODUTOS,
  		 COUNT(DISTINCT PCPEDI.DATA)							AS QTD_DIAS,
  		 COUNT(DISTINCT PCPEDC.DATA)							AS QTD_DIAS2,		-- PELA PCPEDC (PROBLEMA NA TABELA)
  		 MAX(PCPEDI.DATA)										AS ULTIMO_PEDIDO,
  		 MAX(PCPEDC.DATA)										AS ULTIMO_PEDIDO2 	-- PELA PCPEDC (PROBLEMA NA TABELA)
    FROM WINT.PCPEDC
    JOIN WINT.PCPEDI
      ON PCPEDC.NUMPED = PCPEDI.NUMPED
    JOIN WINT.PCUSUARI
